...
export class HttpServiceService {
  ...
  sendNIC(nic: string): Observable<number> {
    const paramss = new HttpParams().set('nic', nic);

    return this.http.post<number>('http://localhost:8080/avs/api/v1/voter/login', '', { params: paramss });
  }

  // OTP send
  sendOTP(otp: string, nic: string): Observable<any> {
    const paramss = new HttpParams().set('otp', otp).set('nic',nic);

    return this.http.post<any>('http://localhost:8080/avs/api/v1/voter/otp-validate', '', { params: paramss });
  }
